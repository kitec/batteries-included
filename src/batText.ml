...
let read_text i n =
  let rec loop r j =
    if j = 0 then r
    else loop (Text.append_char (read_char i) r) (j-1) (* TODO: make more efficient by appending a string of Rope.leaf_size (256) chars at a time *)
  in
  if n <= 0 then Text.empty
  else loop Text.empty n

(** read the whole contents of a UTF-8 encoded input*)
let read_all i = Text.of_string (BatIO.read_all i)
...
let write_text o t = Text.print o t
...
let sprintf fmt =
  BatPrint.ksprintf Text.of_string fmt
